...
public class EnergyOrbBard extends CustomEnergyOrb
{
    ...
    public EnergyOrbBard()
    {
        super(null, null, null);

        fbo = new FrameBuffer(Pixmap.Format.RGBA8888, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), false, false);

        orbVfx = BardMod.assets.loadImage(BardMod.assetPath("images/ui/topPanel/energyBardVFX.png"));

        mask = BardMod.assets.loadImage(BardMod.assetPath("images/ui/topPanel/bard/mask.png"));
        border = BardMod.assets.loadImage(BardMod.assetPath("images/ui/topPanel/bard/border.png"));
        for (int i=0; i<LAYER_COUNT; ++i) {
            layers[i] = BardMod.assets.loadImage(BardMod.assetPath("images/ui/topPanel/bard/" + (i+1) + ".png"));
        }
        for (int i=0; i<LAYER_COUNT; ++i) {
            layers_d[i] = BardMod.assets.loadImage(BardMod.assetPath("images/ui/topPanel/bard/" + (i+1) + "d.png"));
        }
    }
